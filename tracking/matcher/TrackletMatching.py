...
class TrackletMatcher:
    ...
    def get_3d_tracks(self, tracks1, tracks2, pre_3d_tracks):
        ...
        if track_pairs is not None and len(track_pairs) != 0:
            ...
            tracks_3d = self.confirm_tracks(pre_3d_tracks, tracks_3d)
        return tracks_3d

    def confirm_tracks(self, pre_tracks, current_tracks):

        needs_assignment = False

        for pre_obj in pre_tracks:
            matched_obj = next((cur_obj for cur_obj in current_tracks if cur_obj['id'] == pre_obj['id']), None)
            if matched_obj:
                distance = self.euclidean_distance(pre_obj, matched_obj)
                # if distance > pre_obj['err']:
                if distance > 50:
                    needs_assignment = True
                    break

        if needs_assignment:
            distance_matrix = self.calculate_distance_matrix(pre_tracks, current_tracks)
            row_ind, col_ind = linear_sum_assignment(distance_matrix)

            for i, j in zip(row_ind, col_ind):
                current_tracks[j]['id'] = pre_tracks[i]['id']
    ...
    def euclidean_distance(self, point1, point2):
        return math.sqrt((point1['3d_x'] - point2['3d_x']) ** 2 +
                         (point1['3d_y'] - point2['3d_y']) ** 2 +
                         (point1['3d_z'] - point2['3d_z']) ** 2)

    def calculate_distance_matrix(self, list1, list2):
        matrix = np.zeros((len(list1), len(list2)))
        for i, obj1 in enumerate(list1):
            for j, obj2 in enumerate(list2):
                matrix[i][j] = self.euclidean_distance(obj1, obj2)
        return matrix
