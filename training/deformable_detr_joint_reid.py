...
class GenericLoss(torch.nn.Module):
    def __init__(self, opt, weight_dict):
        ...
        self.reid_loss = TripletLoss(margin=0.3)
    ...
    def forward(self, outputs, batch):
        ...
        for s in range(outputs['hm'].shape[0]):
            ...
            if "reid" in outputs:
                head = "reid"
                losses[head + end_str] = self.reid_loss(outputs[head][s], batch[head][s].long())
...
def build(args):
    ...
    # build transformer from ./model/deformable_transformer
...
class TripletLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        dist = dist + dist.t()
        dist.addmm_(inputs.float(), inputs.float().t(), beta=1, alpha=-2)
