...
from models.ops.ops_dcnv3.modules import DCNv3
import math
...
class DeformConv(nn.Module):
    def __init__(self, chi, cho):
        ...
        if chi != cho:
            self.stem = nn.Conv2d(chi, cho, kernel_size=(1, 1), dtype=torch.float32)
        self.conv = DCNv3(cho, kernel_size=3, stride=1, pad=1, dilation=1, group=1)
...
class IDAUpV3_bis(nn.Module):
    ...
    def __init__(self, o, channels, node_type=(DeformConv, DeformConv)):
        ...
        self.up_2 = DySample(in_channels=256, scale=2)  # no params
    ...
    def forward(self, layers, startp, endp):
        for i in range(endp-1, startp, -1):
            ...
            layers[i], coord = self.up_2(layers[i])  # ch 256-> 256
        ...
        coord = coord.clone()
        torch.save(coord, 'sampling_points.pth')
